...
function Changelog() {
  ...
  useEffect(() => {
    ...
    if (!target || visibleCount >= releases.length) return;
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) setVisibleCount((count) => Math.min(count + 4, releases.length));
